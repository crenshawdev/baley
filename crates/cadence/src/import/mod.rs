...
/// Reads the lifecycle documents through the session's own reader, with one
/// guard per document naming the bytes read. Listing and probing a phase
/// directory stay with the derivation's reader.
...
/// The lifecycle documents: the capture derivation works from, and one guard
/// per document naming the bytes read.
