...
#[test]
fn root_identity_reopens_at_same_path_with_matching_bytes_and_legacy_binding() {
    ...
    // Application data need not be an object, and root binding needs no import.
    for data in [json!(7), json!({"fixture": true})] {
        let fixture = external_temp();
        let root = fixture.path().join(".planning");
        let runtime = tokio::runtime::Runtime::new().unwrap();
        let store = runtime.block_on(Store::open(Filesystem::new(&root).unwrap(), Allow)).unwrap();
        let view = runtime.block_on(store.request(Operation::RewriteSnapshot(data.clone()))).unwrap();
        assert_eq!(view.snapshot.generation, 1);
        assert_eq!(view.snapshot.data, data);
        let record = view.snapshot.root.as_ref().unwrap();
        assert_eq!(record.path, root);
        assert_eq!(record.bound, Filesystem::new(&root).unwrap().read(STATE).unwrap().directory_identity);
        assert!(record.relocations.is_empty());
        drop(store);
        let legacy = view.snapshot.clone().with_root(None).unwrap();
        std::fs::write(root.join(STATE), legacy.render().unwrap()).unwrap();
        let store = runtime.block_on(Store::open(Filesystem::new(&root).unwrap(), Allow)).unwrap();
        let migrated = runtime.block_on(store.request(Operation::Read)).unwrap();
        assert_eq!(migrated.snapshot.generation, 2);
        assert_eq!(migrated.snapshot.data, data);
        assert_eq!(migrated.snapshot.root, view.snapshot.root);
    }
    for legacy in [false, true] {
        let fixture = external_temp();
        let root = fixture.path().join(".planning");
        std::fs::create_dir_all(root.join("phases/9")).unwrap();
        std::fs::write(root.join("ROADMAP.md"), "## Phases\n- [x] **Phase 9: Declared**\n").unwrap();
        std::fs::write(root.join("phases/9/PLAN-1.md"), "# Plan\n").unwrap();
        std::fs::write(root.join("config.json"), "{}\n").unwrap();
        let runtime = tokio::runtime::Runtime::new().unwrap();
        let session = runtime.block_on(SessionFactory::new(None, allow_evaluation()).first_touch(&root)).unwrap();
        let view = runtime.block_on(session.request(Operation::Read)).unwrap();
        let record = adoption::applicable(&view.snapshot.data, 9).unwrap().unwrap();
        let bound = record.root_binding.clone();
        drop(session);
        if legacy {
            let mut data = view.snapshot.data.clone();
            data.as_object_mut().unwrap().remove("import");
            let state = Snapshot::new(view.snapshot.generation,
                &std::fs::read(root.join(ITEMS)).unwrap(),
                &std::fs::read(root.join(DECISIONS)).unwrap(), data).unwrap()
                .with_operations(view.snapshot.operations.clone()).unwrap();
            std::fs::write(root.join(STATE), state.render().unwrap()).unwrap();
        }
        let before: Snapshot = serde_json::from_slice(&std::fs::read(root.join(STATE)).unwrap()).unwrap();
        let open = |path: &Path| runtime.block_on(async {
            let store = Store::open(Filesystem::new(path)?, Allow).await?;
            let view = store.request(Operation::Read).await?;
            Ok::<_, Error>((store, view))
        });
        let aside = fixture.path().join("old-planning");
        std::fs::rename(&root, &aside).unwrap();
        copy_tree(&aside, &root);
        let live = Filesystem::new(&root).unwrap().read(STATE).unwrap().directory_identity;
        assert_ne!(live, bound);
        let (store, moved) = open(&root).unwrap();
        assert_eq!(adoption::applicable(&moved.snapshot.data, 9).unwrap(), Some(record));
        assert_eq!(cadence::verification::inputs::root_binding(&root).unwrap(), bound);
        assert_eq!(moved.snapshot.generation, view.snapshot.generation + 1);
        assert_eq!(cadence::verification::inputs::authority_digest(&moved.snapshot.data).unwrap(),
            cadence::verification::inputs::authority_digest(&before.data).unwrap());
        assert_eq!(serde_json::to_value(&moved.snapshot.root).unwrap(), json!({"bound":bound,"path":root,
            "relocations":[{"from":bound,"to":live,"generation":moved.snapshot.generation}]}));
        drop(store);
        let (store, again) = open(&root).unwrap();
        assert_eq!(again, moved);
        drop(store);
        let other = fixture.path().join("other-planning");
        copy_tree(&root, &other);
        let other_chain = Filesystem::new(&other).unwrap().read(STATE).unwrap().directory_identity;
        let refused = open(&other).err().unwrap().to_string();
        for detail in [&bound, &other_chain, other.to_str().unwrap()] {
            assert!(refused.contains(detail), "{refused}");
        }
        std::fs::remove_dir_all(&root).unwrap();
        copy_tree(&aside, &root);
        let bad_chain = Filesystem::new(&root).unwrap().read(STATE).unwrap().directory_identity;
        if legacy {
            let bytes = std::fs::read(root.join(STATE)).unwrap();
            let mut state: Snapshot = serde_json::from_slice(&bytes).unwrap();
            let records = state.data["adoption"]["declared_completions"].as_array_mut().unwrap();
            let mut conflicting = records[0].clone();
            conflicting["root_binding"] = json!("disagreeing-chain;");
            records.push(conflicting);
            let state = Snapshot::new(state.generation,
                &std::fs::read(root.join(ITEMS)).unwrap(),
                &std::fs::read(root.join(DECISIONS)).unwrap(), state.data).unwrap()
                .with_operations(state.operations).unwrap();
            std::fs::write(root.join(STATE), state.render().unwrap()).unwrap();
            let refused = open(&root).err().unwrap().to_string();
            for detail in ["disagree", &bound, "disagreeing-chain;"] {
                assert!(refused.contains(detail), "{refused}");
            }
            std::fs::write(root.join(STATE), bytes).unwrap();
        }
        let mut items = std::fs::read(root.join(ITEMS)).unwrap();
        items.push(b' ');
        std::fs::write(root.join(ITEMS), items).unwrap();
        let refused = open(&root).err().unwrap().to_string();
        for detail in [&bound, &bad_chain, root.to_str().unwrap()] {
            assert!(refused.contains(detail), "{refused}");
        }
    }
}
