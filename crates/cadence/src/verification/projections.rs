...
//!
//! UAT.md for a native phase is the native human results. Every renderer here
//! is a pure function of the snapshot so commit and recovery derive the same
//! bytes.
use super::human;
...
/// None when the phase has no native result.
pub fn uat(data: &Value, phase: u32) -> Result<Option<String>> {
    let history = human::records(data)?;
    if !history.iter().any(|r| r.submission.phase == phase) {
        return Ok(None);
    }
    ...
    for (index, item) in human::items(data, phase)?.iter().enumerate() {
