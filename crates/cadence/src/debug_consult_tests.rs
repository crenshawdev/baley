...
async fn read(factory: &SessionFactory, root: &Path, process: &mut (dyn Process + Send)) -> Value {
    execute(factory, root, Command::Read { slug: "tenant-cache".into() }, process).await.unwrap()
}
async fn call(factory: &SessionFactory, root: &Path, operation: &str, request: Value, process: &mut (dyn Process + Send)) -> Value {
    execute(factory, root, Command::Apply(serde_json::from_value(json!({"operation":operation,"request":request})).unwrap()), process).await.unwrap()
}
async fn change(factory: &SessionFactory, root: &Path, operation: &str, id: &str, mut fields: Value, process: &mut (dyn Process + Send)) -> Value {
    let version = if operation == "debug-open" { json!(0) } else { read(factory, root, process).await["record"]["version"].clone() };
    fields["request_id"] = json!(id); fields["slug"] = json!("tenant-cache"); fields["expected_version"] = version;
    let answer = call(factory, root, operation, fields, process).await;
    assert_eq!(answer["status"], "ok", "{answer}"); answer
}
async fn failed(factory: &SessionFactory, root: &Path, id: &str, process: &mut (dyn Process + Send)) -> Value {
    change(factory, root, "debug-attempt", id, json!({"attempt":{"description":"flush cache","result":"still shares tenants"}}), process).await
}
...
async fn debug_consult_is_offered_once_per_dead_end() {
    ...
    ENVIRONMENT.scope(environment, async {
        for (enabled, model) in [(false,true),(true,false)] {
            ...
            change(&factory, &root, "debug-open", "open", json!({"symptom":"tenant cache leaks entries"}), process).await;
            failed(&factory, &root, "failed-one", process).await;
            assert!(offers(&failed(&factory, &root, "failed-two", process).await).is_empty());
            assert!(wire.requests.lock().unwrap().is_empty());
        }
        let (_tree, root, mut service) = fixture(true, true, None).await;
        let opened = change(&service, &root, "debug-open", "open", json!({"symptom":"tenant cache leaks entries; API_KEY=secret-value"}), process).await;
        assert!(offers(&opened).is_empty(), "empty hypotheses are not a dead end");
        assert!(offers(&failed(&service, &root, "failed-one", process).await).is_empty());
        let offered = failed(&service, &root, "failed-two", process).await;
        ...
        let third = failed(&service, &root, "failed-three", process).await;
        assert_eq!(offers(&third), offers(&offered));
        drop(service); service = factory();
        for _ in 0..3 { assert_eq!(offers(&read(&service, &root, process).await), offers(&offered)); }
        let request = decision(&read(&service, &root, process).await, "accept-first", "accept");
        let accepted = call(&service, &root, "debug-consult", request.clone(), process).await;
        ...
        assert_eq!(call(&service, &root, "debug-consult", request.clone(), process).await, accepted);
        let mut changed = request; changed["decision"] = json!("decline");
        assert_eq!(call(&service, &root, "debug-consult", changed, process).await["code"], "request-reused");
        ...
        drop(service); service = factory();
        let resumed = read(&service, &root, process).await;
        ...
        assert_eq!(offers(&failed(&service, &root, "failed-four", process).await).len(), 1);
        let observation = json!({"observation":{"test":"compare request logs","result":"two tenant ids share a key","rules_in":[],"rules_out":[]}});
        let next = change(&service, &root, "debug-observation", "new-evidence", observation.clone(), process).await;
        assert_eq!(offers(&next).len(), 2);
        assert_ne!(offers(&next)[0]["epoch"], offers(&next)[1]["epoch"]);
        let duplicate = change(&service, &root, "debug-observation", "duplicate-evidence", observation, process).await;
        assert_eq!(offers(&duplicate).len(), 2, "identical evidence does not open another epoch");
        let second = call(&service, &root, "debug-consult", decision(&duplicate, "accept-second", "accept"), process).await;
        ...
        change(&declined_service, &decline_root, "debug-open", "open", json!({"symptom":"tenant cache"}), process).await;
        for (id, state) in [("h-one","untested"),("h-two","untested"),("h-one","refuted"),("h-two","refuted")] {
            let answer = change(&declined_service, &decline_root, "debug-hypothesis", &format!("{id}-{state}"),
                json!({"hypothesis":{"id":id,"description":"candidate cause","rank_reason":"cheap test","state":state}}),
                process,).await;
            assert_eq!(offers(&answer).len(), usize::from(id == "h-two" && state == "refuted"));
        }
        let refuted = read(&declined_service, &decline_root, process).await;
        assert_eq!(refuted["record"]["attempt_count"], 0);
        let decline_request = decision(&refuted, "decline", "decline");
        let declined = call(&declined_service, &decline_root, "debug-consult", decline_request.clone(), process).await;
        assert_eq!(offers(&declined)[0]["state"], "declined");
        assert_eq!(call(&declined_service, &decline_root, "debug-consult", decline_request, process).await, declined);
        failed(&declined_service, &decline_root, "failed-after-decline", process).await;
        drop(declined_service);
        assert_eq!(offers(&read(&factory(), &decline_root, process).await), offers(&declined));
        assert_eq!(wire.requests.lock().unwrap().len(), 2);

        let (_cap_tree, cap_root, capped) = fixture(true, true, Some(1)).await;
        change(&capped, &cap_root, "debug-open", "open", json!({"symptom":"tenant cache leaks entries"}), process).await;
        failed(&capped, &cap_root, "failed-one", process).await;
        let offered = failed(&capped, &cap_root, "failed-two", process).await;
        ...
        let accepted = call(&capped, &cap_root, "debug-consult", decision(&offered, "accept-capped", "accept"), process).await;
        ...
        drop(capped);
        assert_eq!(offers(&read(&factory(), &cap_root, process).await), offers(&accepted), "cap failure must survive restart");
        ...
        let next = change(&service, &root, "debug-observation", "later-evidence",
            json!({"observation":{"test":"inspect keys","result":"key format measured","rules_in":[],"rules_out":[]}}),
            process,).await;
        let request = decision(&next, "uncertain-accept", "accept");
        wire.hold.store(true, std::sync::atomic::Ordering::SeqCst);
        let mut pending = Box::pin(call(&service, &root, "debug-consult", request.clone(), process));
        ...
        assert_eq!(
            call(&service, &root, "debug-consult", request, &mut cadence::process::System).await
                ["code"],
