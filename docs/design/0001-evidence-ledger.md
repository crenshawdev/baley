...
| Settings layers merge, project over global | Configuration and routing [TARGET] | `policy.effective` with the precedence and scope the configuration design defines |
| A dispatch whose routing inputs changed is refused | Configuration and routing [TARGET] | Kept; routing inputs are part of the policy version bound at admission |
