...
| A building marker names the live generation | Every removal turn checks `live_gen` before deleting a row | `rebuild`, or a read or command that must first rebuild the project, returns `LiveGenerationProtected`; nothing is removed and the live views stand | Restore the latest verified backup |
