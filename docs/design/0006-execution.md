...
It does not decide the plan's content or its evidence map ([0005](0005-context-plans-and-acceptance.md)); the verdict on each evidence item and a truth's status ([0007: Verification](0007-verification.md) [TARGET]); when a diff review or a risk gate fires after execution ([0008](0008-review.md) [TARGET], [0009](0009-risk.md) [TARGET]); what the guard does with git commands and protected branches ([0010: Guard](0010-guard.md) [TARGET]); which branch work lands on and how execution is undone ([0011](0011-milestones-landing-undo-pause.md) [TARGET]); or how a work order reaches a worker on each host ([0012](0012-host-interface.md) [TARGET]). Off-roadmap tasks (`baley task`) are [0014](0014-support-families.md) [TARGET].
...
| `git.protected_branches`, `git.on_protected`, `git.auto_branch`, `git.base_branch` | see [0003](0003-configuration-and-routing.md) | | project | 0010, 0011 [TARGET] | Which branch a dispatch works on and what the guard does there |
...
| Question | Decided by |
|---|---|
| Whether Codex runs a hook before `apply_patch`, which decides whether the guard's early lease stop exists there | [0010: Guard](0010-guard.md) [TARGET], by a probe on Codex |
